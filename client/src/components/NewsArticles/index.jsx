    ...
    if(props.data){
        const results = props.data.map((result, i) => (
            ...
            <MDBCol md="4" className="float-left">
                <MDBCard className="shadow-box-example z-depth-2" style={{ width: "25rem", height: "550px", marginBottom: "50px" }}>
                    ...
                    <MDBCardTitle style={{ fontSize: "12px" }}>{result['date'].slice(0, 14)}</MDBCardTitle>
                        <MDBCardTitle style={{ fontSize: "20px"}}>{result['title']}</MDBCardTitle>                       
                            ...
                            <MDBBtn  style={{marginTop:'50px'}} size="lg" gradient="blue"  href={result['news_url']} target="_blank">Read Article <MDBIcon icon="glasses" className="ml-1" /></MDBBtn>
                            </MDBContainer>
                    </MDBCardBody>
                </MDBCard>
            </MDBCol>
